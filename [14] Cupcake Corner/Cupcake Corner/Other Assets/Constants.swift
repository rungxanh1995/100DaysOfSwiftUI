...
struct Constants {
	...
	struct ImageUrl {
		static var randomCupcake: URL? {
			let randomIndex = Int.random(in: 0..<Self.decorativeCupcakes.count)
			return Self.decorativeCupcakes[randomIndex]
		}
		
		private static let decorativeCupcakes: [URL?] = [
